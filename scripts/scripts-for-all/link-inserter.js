function replaceWordsWithLinks(rootNode = document.body) {
    fetch('/articles/pages.json')
        ...
        .then(pages => {
            ...
            // 🔹 Added support for "@Name" at the end of the regex
            const pattern = /\(([^\)]+)\)\s*@\(([^\)]+)\)|([\p{L}\p{N}_\-]+)\s*@\(([^\)]+)\)|([\p{L}\p{N}_\-]+)\s*@([\p{L}\p{N}_\-]+)|([\p{L}\p{N}_\-]+)@([\p{L}\p{N}_\-]+)|@([\p{L}\p{N}_\-]+)/gu;
            ...
            for (const node of nodes) {
                ...
                const newHtml = text.replace(pattern, (
                    ...
                    const targetName = (targetParen1 || targetParen2 || targetSingle1 || targetSingle2 || atOnlyTarget || '').trim();
